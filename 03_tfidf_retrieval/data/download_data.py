...
TARGET_NAME = "News_Category_Dataset_v3.json"


def find_dataset_file(folder: Path):
    matches = list(folder.rglob(TARGET_NAME))
    if matches:
        return matches[0]
    matches = list(folder.rglob("News_Category_Dataset*.json"))
...
def main():
    ...
    try:
        ...
        source = find_dataset_file(downloaded)
        if source:
    ...
    source = find_dataset_file(DATA_DIR)
    if not source:
